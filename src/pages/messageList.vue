...
export default {
    ...
    methods:{
		 ...
		 */
		getFriendsInGlohelp(){
			this.axios.get(this.ip+'/globalmate/rest/userRelation/getFriends?token='+this.$route.query.token+'&userId='+this.CURRENTUSER.id,{
		...
		processFriendsList(friends){
			...
			for(var i=0;i<mess.length;i++){
				let temp=mess[i];
				this.axios.get(this.ip+'/globalmate/rest/user/list/'+mess[i].id+'?token='+this.$route.query.token,{}).then((res)=>{
					if(res.success){
		...
		getUserByToken(){
			let _this=this;
			this.axios.get(this.ip+'/globalmate/rest/user/getUserByToken'+'?token='+_this.$route.query.token,{}).then((res)=>{
				if(res.success){
