...
export default {
          ...
          .then(res => {
            if (res.success) {
              if (res.data) {
                ...
                temp.lastContactTime = this.moment(temp.lastContactTime).format(
                  "YYYY-MM-DD"
                );
      ...
      if (message && message.from) {
        this.list.forEach(function(item, index) {
          if (item.id == message.from && message.data.content) {
            try {
              item.lastMessageContent = JSON.parse(
                message.data.content
              ).chatContent;
            } catch (e) {
              item.lastMessageContent = message.data.content;
            }
            item.lastContactTime = _this
              .moment(message.dateline)
              .format("YYYY-MM-DD");
  ...
  activated() {
    ...
    // 	this.getUserByToken();
    // }else{
    // 	this.CURRENTUSER=.getItem('gl_CURRENTUSER');
    // }
