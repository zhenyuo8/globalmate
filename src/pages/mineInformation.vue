...
.mineInformation_hobby_warp .mineInformation_hobby_header {
  margin-bottom: 0;
  text-align: left;
}
      ...
      <div class="mineInformation_detail">
        ...
        <div class="mineInformation_information">
          <div class="mineInformation_top">
            ...
            <span class="mineInformation_call">{{$t('formTitle.serviceValue')}}
              <i style='color:#ff0023'>{{nice}}</i>
            </span>
          </div>
        </div>
        ...
        success: function(data) {
          if (data) {
            ...
            if (isFriend) {
              Toast({
                message:
                  "您和" + _this.information.nikename + "已经是好友关系了！",
                duration: 2000
              });
            } else {
              MessageBox.confirm("", {
...
>>>>>>> 1a7d1b54530a47b84652e45b598478e9434f26fe
        },
        error: function(err) {
          console.log(err);
        },
    ...
    getEvalutePic(data) {
      ...
      for (var i = 0; i < data.length; i++) {
        var curData = data[i];
        curData.evaluation.createTime = this.moment(
          curData.evaluation.createTime
        ).format("YYYY-MM-DD");
        for (var n = 0; n < this.userList.length; n++) {
          if (curData.evaluation.uEvaluatorId == this.userList[n].id) {
            curData.pic = this.userList[n].pic;
            ...
            for (var i = 0; i < len; i++) {
              ...
              for (var j = i + 1; j < len; j++) {
                if (
                  _this.commentList[j].evaluation.score >
                  _this.commentList[minIndex].evaluation.score
                ) {
                  minIndex = j;
                }
              }
