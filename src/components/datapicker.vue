<template>
  <div class="cov-vue-date">
    <div class="datepickbox">
      ...
      <div class="cov-datepicker" readonly="readonly" @click='showCheck' :style="option.inputStyle ? option.inputStyle : {}" :placeholder="option.placeholder"></div>
    </div>
      ...
      <div class="cov-date-body" :style="{'background-color': option.color ? option.color.header : '#3f51b5'}">
        ...
        <div class="cov-date-box" v-if="showInfo.day">
          <div class="cov-picker-box">
            ...
            <div class="day" v-for="(day, index) in dayList" track-by="$index" @click="checkDay(day)" :class="{'checked':day.checked,'unavailable':day.unavailable,'passive-day': !(day.inMonth)}" :style="day.checked ? (option.color && option.color.checkedDay ? { background: option.color.checkedDay } : { background: '#F50057' }) : {}" :key='index'>{{day.value}}</div>
          </div>
        </div>
  ...
  methods: {
    ...
    nextMonth: function nextMonth(type) {
      ...
      type === "next"
        ...
        : (next = (0, _moment2.default)(this.checked.currentMoment).add(
            -1,
            "M"
          ));
      this.showDay(next);
    ...
    showDay: function showDay(time) {
      ...
      this.showOne("day");
      this.checked.year = (0, _moment2.default)(
        this.checked.currentMoment
      ).format("YYYY");
      this.checked.month = (0, _moment2.default)(
        this.checked.currentMoment
      ).format("MM");
      this.checked.day = (0, _moment2.default)(
        this.checked.currentMoment
      ).format("DD");
      this.displayInfo.month = this.library.month[
        (0, _moment2.default)(this.checked.currentMoment).month()
      ];
      var days = [];
      var currentMoment = this.checked.currentMoment;
      var firstDay = (0, _moment2.default)(currentMoment)
        .date(1)
        .day();
      ...
      for (var i = 1; i <= monthDays; ++i) {
        ...
        if (
          i === Math.ceil((0, _moment2.default)(currentMoment).format("D")) &&
          (0, _moment2.default)(oldtime, this.option.format).year() ===
            (0, _moment2.default)(currentMoment).year() &&
          (0, _moment2.default)(oldtime, this.option.format).month() ===
            (0, _moment2.default)(currentMoment).month()
        ) {
      ...
      for (
        var _i = 0;
        _i < firstDay - (this.option.SundayFirst ? 0 : 1);
        _i++
      ) {
        var passiveDay = {
          ...
          unavailable: false,
          checked: false,
          moment: (0, _moment2.default)(currentMoment)
            .date(1)
            .subtract(_i + 1, "days")
        };
      ...
      if (this.limit.length > 0) {
        ...
        try {
          for (
            var _iterator = this.limit[Symbol.iterator](), _step;
    ...
    checkBySelectDays: function checkBySelectDays(d, days) {
      ...
      this.selectedDays.forEach(function(day) {
        if (
          _this.checked.year === (0, _moment2.default)(day).format("YYYY") &&
    ...
    getLimitCondition: function getLimitCondition(limit, day) {
      var tmpMoment = (0, _moment2.default)(
        this.checked.year +
          "-" +
    ...
    handleStatus() {
      let ts1 = new Date(this.selectedDays[0]).getTime();
      let ts2 = new Date(this.selectedDays[1]).getTime();
      this.dayList.forEach(item => {
        let moment = item.moment;
        if (moment && moment._d) {
          let time = moment._d.getTime();
          if (time > ts1 && time < ts2) {
            item.checked = true;
          }
        }
      });
    ...
    checkDay(obj) {
      ...
      if (this.option.type === "day" || this.option.type === "min") {
        ...
        if (this.selectedDays.length === 0) {
          ...
          if (ctime !== this.selectedDays[0]) {
            let cts = new Date(ctime).getTime();
            let startTime = new Date(this.selectedDays[0]).getTime();
            if (cts < startTime) {
              this.dayList.forEach(item => {
                item.checked = false;
              });
              obj.checked = true;
              this.selectedDays[0] = ctime;
            } else {
              obj.checked = true;
              this.selectedDays.push(ctime);
              this.handleStatus();
              // if (m1 === m2) {
              //   this.dayList.forEach(item => {
              //     if (item.inMonth === true && item.value < day2 && item.value > day1) {
              //       item.checked = true
              //     }
              //   })
              // } else if (m1 === m2 - 1) {
              //   this.dayList.forEach(item => {
              //     // if (item.inMonth === false && )
              //   })
              // } else if (m1 < m2) {
              //   this.dayList.forEach(item => {
              //     if (item.inMonth === false) {
    ...
    showCheck: function showCheck() {
      if (this.date.time === "") {
        ...
        if (this.option.type === "day" || this.option.type === "min") {
          ...
          this.selectedDays = this.date.time;
    ...
    picked: function picked() {
      if (this.option.type === "day" || this.option.type === "min") {
        ...
        this.date.time = (0, _moment2.default)(
          this.checked.currentMoment
        ).format(this.option.format);
      } else {
        // this.date.time = JSON.stringify(this.selectedDays);
        this.date.time = this.selectedDays;
      }
